...
public class RegisterApiControllerTest {
	...
	public void customerRegisterTest() throws Exception {
		...
		MockMultipartFile jsonFile = new MockMultipartFile("customer", "", "application/json", getCustomerRequest().getBytes());
		LoginRequest res = new LoginRequest("S.Sanniboena", "gBE3XgRWjZ");
	...
	public void ratelimiterTest() throws Exception {
		MockMultipartFile file = new MockMultipartFile("file", "filename.txt", "text/plain", "some text".getBytes());
		 ResponseEntity<String> res=registerApiController.rigisterRateLimitingFallback(new Customer(), file,null);
